...
종료 방식 중 하나 이상을 바꾸도록 한 번 재계획·재생성한다. 두 번째 결과에도
경계선상의 유사도 신호가 남으면 상태의 반복 경고에 기록해 다음 계획에 반영한다.
형식 오류, 잘린 JSON, 필수 장면 누락은 실패 사유를 다음 요청에 직접 전달하고,
같은 모델에서 해결되지 않으면 검증된 다른 텍스트 모델로 전환한다.
분당 할당량 응답에 재시도 대기 시간이 포함되면 고정된 짧은 백오프 대신 해당
시간을 기다려 같은 할당량 구간에서 연속 실패하는 일을 피한다.
대기 뒤에도 모델별 할당량이 회복되지 않으면 본문뿐 아니라 계획·재계획 단계도
다음 텍스트 모델로 전환한다.

정상 품질 경로의 LLM 호출은 모델 probe를 제외하고 계획 1회와 본문 1회다.
형식·연속성·반복 위험이 있을 때만 제한된 재시도와 모델 전환을 사용한다.
API의 일시 오류 재시도는 별도의 제한된 지수 백오프를 사용한다.
...
매일 KST 03:17, 07:47, 12:17, 18:47에 독립적인 생성 기회를 둔다.
첫 성공 뒤 같은 KST 날짜의 실행은 건너뛴다. GitHub Actions의 예약 작업은
부하에 따라 실제 시작이 지연될 수 있다.
...
submit은 사용하지 않는다. 일반 문단은 viewer CSS의 들여쓰기와 여백이 중복되지
않도록 `<p>`나 빈 spacer 문단 대신 `<br><br>`로 구분한다. 입력 전 우발적인
선행 공백·탭·과도한 빈 줄을 정규화하고, 입력 후에는 문단 간격 손실과 인위적인
공백 들여쓰기가 없는지 확인한 뒤에만 게시한다.
